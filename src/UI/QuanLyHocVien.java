...
public class QuanLyHocVien extends javax.swing.JFrame {
    ...
    //lấy tất cả đối tượng HocVien không thuộc khoaHoc từ CSDL (theo maKH)
    ...
    void fillComboBox() {
        ...
        try {
            //lấy tất cả đối tượng HocVien không thuộc khoaHoc từ CSDL
            //rồi thêm vào combobox
            List<HocVien> list = nhdao.selectByCourse(MaKH);
            for (HocVien nh : list) {
                model.addElement(nh);
            }
        } catch (Exception e) {
    ...
    /*
    thêm đt NguoiHoc vào CSDL bảng NguoiHoc
     ...
     */
    void insert() {
        HocVien nguoiHoc = (HocVien) cboNguoiHoc.getSelectedItem(); //lấy đt HocVien từ combobox
        NguoiHoc model = new NguoiHoc(); //tạo đt NguoiHoc
        model.setMaKH(MaKH);
        model.setMaNH(nguoiHoc.getMaNH());
        model.setDiem(Double.valueOf(txtDiem.getText()));
        try {
            dao.insert(model);  //thêm đt NguoiHoc vào CSDL bảng NguoiHoc
    ...
    //chỉ sửa điểm NguoiHoc, xóa NguoiHoc bị tích
